...
- As a post-implementation review after finishing a coding/application task, to check whether duplicated operational logic should be extracted into a shared service layer before the work is considered done

**Don't use when:** Logic is truly domain-specific and used by only one caller.

## Post-Implementation Review Trigger

After shipping or substantially editing an application feature, do a quick code-structure pass before finalizing:

1. Scan the changed flow(s) for repeated operational mechanics across actions/controllers/routes/jobs.
2. Separate domain orchestration (policy, auth, state transitions, user-facing errors) from reusable mechanics (provider calls, sandbox/bootstrap/build/startup helpers, transport logic).
3. If the same mechanic appears in 2+ places or is likely to recur in the next nearby feature, extract or propose a composable service-layer function.
4. If the logic is still single-use and tightly coupled to one workflow, leave it in the action and explicitly avoid premature abstraction.

This keeps the skill useful both during refactors and as a final architectural review step after implementation.
