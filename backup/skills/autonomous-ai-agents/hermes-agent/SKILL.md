...
#### Browser Use MCP example

Browser Use Cloud exposes an MCP endpoint at:

```text
https://api.browser-use.com/v3/mcp
```

A practical Hermes config looks like:
...
Typical tools exposed by Browser Use MCP include `run_session`, `send_task`, `get_session`, `get_session_messages`, `stop_session`, `list_sessions`, and `list_browser_profiles`.

Important operational notes:
- New MCP servers are only loaded on a fresh Hermes process/session. In gateway use `/restart`; in chat use `/reset` only after the server is already part of startup config.
- Hermes resolves `${ENV_VAR}` placeholders in `mcp_servers.*.headers`, so prefer env interpolation over hardcoding API keys into `config.yaml`.
- If `hermes mcp test` says the account has insufficient balance, that is a Browser Use Cloud account/project billing issue rather than a Hermes MCP wiring failure.
- If MCP discovery appears unavailable, ensure the Hermes runtime venv has the `mcp` Python package installed/upgraded.
- See `references/browser-use-mcp-and-live-view.md` for a worked example plus a local noVNC live-view recipe.

Browser Use Cloud note: its MCP endpoint uses a custom header (`x-browser-use-api-key`), so direct `mcp_servers` config in `~/.hermes/config.yaml` is often more reliable than the interactive `hermes mcp add` flow for that provider. See `references/browser-use-mcp-and-live-view.md` for a working config, verification commands, billing pitfall, and a self-hosted live noVNC browser-view recipe.

If the user wants Browser Use integrated into Hermes, or wants a self-hosted watchable URL for a local browser session, see `references/browser-use-mcp-and-live-view.md`. It covers the working `mcp_servers.browser-use` config, the need to install `mcp` in Hermes's own venv, and a noVNC + localhost.run live-view setup.
...
```
hermes memory setup         Configure an external memory provider plugin
hermes memory status        Show built-in + external memory status
hermes memory off           Disable external provider (built-in memory remains active)
hermes memory reset         Erase built-in MEMORY.md and USER.md entries
```

Important semantics:
- Built-in memory is always active unless you explicitly reset it.
- Only one external memory provider can be active at a time.
- Session transcripts, built-in memory, and skills are separate persistence layers:
  - `~/.hermes/sessions/` = full conversation history / transcripts
  - built-in memory = compact durable facts about the user and environment
  - `~/.hermes/skills/` = reusable workflows and procedures
- Vector/external memory improves retrieval; it is not a zero-cost replacement for session storage. If you store raw text plus embeddings, total storage usually increases rather than decreases.
- If the user asks about memory backends, vector storage, or self-hosted Honcho, see `references/memory-backends-and-local-hosting.md` for a concise decision guide and CPU-only hosting notes.
- For a concrete CPU-only Honcho recipe using local embeddings via Ollama (`nomic-embed-text`) plus the minimum local text model needed for Honcho to boot, see `references/honcho-local-embeddings-cpu.md`.
...
`hermes update` does more than a plain package upgrade: on source/git installs it can stash local changes, pull upstream, restore your changes on top, refresh Python/Node dependencies, rebuild the web UI, sync bundled skills, check config migrations, and restart the gateway if installed.

Practical post-update checks:
...
If the updater reports that **local changes were restored on top of updated code**, verify the repo state before assuming everything is clean:

```bash
git -C /usr/local/lib/hermes-agent status
git -C /usr/local/lib/hermes-agent diff
```

If Hermes behaves oddly after an update, inspect those diffs first — restored local modifications are a more likely cause than the updater itself.
...
For local/self-hosted inference servers that expose an OpenAI-compatible API — especially LM Studio — see `references/lm-studio-local-models.md` for a concrete recipe. The common local setup is `http://127.0.0.1:1234/v1` with a non-empty placeholder `OPENAI_API_KEY`, but the model name must exactly match what the local server exposes, and `127.0.0.1` only works when Hermes runs on the same machine.

For OpenRouter free-tier routing and strongest-first fallback chains, see `references/openrouter-free-fallback-chain.md`. It covers how to query the live OpenRouter model catalog, identify currently free models from pricing, configure `fallback_providers`, and verify the chain with `hermes fallback list`.

If the user wants Honcho memory involved before fallback switches, see `references/honcho-pre-fallback-compression.md`. Important nuance: Honcho is an exclusive memory-provider plugin, so you enable it via `memory.provider: honcho` / `hermes memory setup` / `hermes honcho ...` — not via `hermes plugins enable honcho`.

If the user wants Hermes to call Browser Use Cloud via MCP, see `references/browser-use-cloud-mcp.md`. Important nuance: Browser Use MCP expects the header `x-browser-use-api-key`, so the fastest reliable setup is a manual `mcp_servers.browser-use` config entry plus an env var such as `MCP_BROWSER_USE_API_KEY`; Hermes's generic `hermes mcp add ... --auth header` flow defaults to `Authorization: Bearer ...`, which is not the Browser Use header shape.
...
- **Gateway-independent long jobs:** if a background job must survive `hermes gateway restart`, do not start it with the gateway-bound terminal/process manager. Launch it as an OS service, e.g. `systemd-run --unit hermes-task-NAME --working-directory /path hermes chat -Q --provider PROVIDER -m MODEL -s skills -t toolsets -q "..."`, or use the local helper `/root/.hermes/scripts/hermes_independent_task.py start ...`. Pin `--provider` and `-m` per job so plan changes can use a different model without changing the live chat model.
