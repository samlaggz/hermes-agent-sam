...
For a practical note on replacing local CPU-bound Honcho models with managed OpenAI models, see `references/openai-managed-models.md`.
That reference also covers the populated-table migration pitfall when switching embedding model families and the non-destructive `text-embedding-3-small` dimensions workaround.
For the follow-on cleanup path when a deployment already has mixed old/new vectors, see `references/reembedding-reset.md` for a backup + clear + regenerate workflow.
...
When a user asks to "save this in Honcho" and the material is highly sensitive (for example bank transactions, statement contents, card activity, government IDs, passwords, OTPs, or other financial-account records), do **not** store the raw details in Honcho conclusions.

Instead:
- Save only a **preference-level or workflow-level conclusion** in Honcho, such as that the user wants future accounting artifacts preserved for follow-up.
- Keep the sensitive operational artifact in a **local file** or another user-controlled store, and reference it in your normal reply rather than encoding the contents into long-term memory.
- If you need persistence, prefer a concise local inventory/ledger file over a detailed Honcho conclusion.

This keeps Honcho useful for durable preferences without turning long-term memory into a store of raw financial records.
